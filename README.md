...
## Models
We tested three Binary Classifiers, namely Logistic Regression (LR), Decision Trees (DT) and Support Vector Machine (SVM). The ```ModelTest.ipynb``` notebook trains and tests each model on all the datasets, to obtain the results that we report in our manuscript.
